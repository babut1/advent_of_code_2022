...
class tree:
    def __init__(self, root):
        ...
        self.smallest_deletable_directory_size = 70000000
    ...
    def get_required_still_needed_space(self):
        return self.root.size - 40000000

    def search_for_the_smallest_deletable_directory(self, current_dir, minimum):
        if current_dir.size > self.get_required_still_needed_space():
            if current_dir.size < minimum:
                minimum = current_dir.size
                self.smallest_deletable_directory_size = minimum
            for kid in current_dir.kids:
                current_dir = kid
                self.search_for_the_smallest_deletable_directory(current_dir, minimum)
        return self.smallest_deletable_directory_size
...
def puzzle_2():
    ...
    result = dir_tree.search_for_the_smallest_deletable_directory(dir_tree.root, dir_tree.smallest_deletable_directory_size)
    return result
